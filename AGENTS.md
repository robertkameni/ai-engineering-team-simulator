...
| Rate limits | Upstash Redis (`@upstash/ratelimit`) on `POST /api/simulate`, `DELETE /api/runs/[id]`, `GET /api/runs/[id]/export`, `GET /api/runs/[id]/export/pdf`, `POST /api/export/pdf`, and `POST /api/runs/[id]/artifacts` — `src/lib/rate-limit.ts`; markdown + PDF exports share the `export_pdf` bucket (5/hr); disabled in dev unless `RATE_LIMIT_ENABLED_IN_DEV=true` |
...
| Usage / cost | `RunUsageAccumulator` + `src/ai/pricing.ts` — persist `promptTokens`, `completionTokens`, `totalTokens`, `estimatedCostUsd` on `Run` via `setRunUsageTotals()`. **Debate cost ceiling:** `assertSimulationWithinBudget()` in `src/ai/orchestration/simulation-budget.ts` aborts the debate loop when `estimatedCostUsd` ≥ limit (env `SIMULATION_MAX_COST_USD`, default **$0.75**); run → `failed`, usage persisted. Checks after classification, each agent turn, and reviewer tag recovery. Artifact synthesis after debate is not capped by this limit. |
...
- **SSE events:** `run_started`, `team_ready`, `agent_start`, `text-delta`, `tool_start`, `tool_end`, `agent_end`, `artifacts_start`, `done`, `error` — shapes in `src/lib/simulation-stream.ts`. Emit `team_ready` right after classification. Artifact synthesis completes in `/api/simulate` before `done`. Non-abort failures emit generic `error.message` (`"Simulation failed"`); internal details logged server-side only.
...
- Env: `.env.local` at repo root (Prisma CLI loads it via `prisma.config.ts`). Production requires `AUTH_SECRET`; rate limits require Upstash Redis env vars. Optional `SIMULATION_MAX_COST_USD` (positive number, USD) overrides the per-run debate cost ceiling (default $0.75).
