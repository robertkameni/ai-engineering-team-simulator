...
export function buildReviewerTurnPrompt(
  ...
  if (options?.isReReview) {
    return `${base}\n\nRE-REVIEW: Evaluate whether the rejected agent provided concrete, testable changes addressing each of your prior objections. If the agent cited specific mitigations, interfaces, configs, or acceptance checks that resolve your objections, you MUST issue [APPROVE] on this line alone — do not reject for new nitpicks or style preferences. Reject only when a named concern from your prior review is still missing from the agent's latest message. Do not claim "all resolved" while any prior objection remains open.`;
  }
