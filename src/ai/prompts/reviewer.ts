...
export function buildReviewerTurnPrompt(
  ...
  if (options?.isReReview) {
    return `SCOPED RE-REVIEW (roughly 100–160 words): Judge only the assigned CRITICAL issue IDs from the scoped checklist. Do not rerun the full first-pass review or invent unrelated blockers. Non-critical observations must not trigger reject. If each assigned open issue now has a concrete mitigation in the correction, emit [APPROVE] alone on the last line. Reject only when a listed CRITICAL issue ID remains unresolved. End with [APPROVE] or [REJECT: role] alone on the absolute last line — no prose after the tag.`;
  }
