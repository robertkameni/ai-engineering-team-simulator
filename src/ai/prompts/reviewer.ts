...
export function buildReviewerTurnPrompt(roster: TeamRoster): string {
  return `Write a short review. Quote at least two claims from the previous agents. ${buildImplementationQuoteHint(roster)} Stay under 220 words. End with [APPROVE] or [REJECT: role] on its own last line (role = pm, architect, backend, or frontend).`;
}
