...
export function buildReviewerTurnPrompt(roster: TeamRoster): string {
  return `Write a short review. Quote at least two claims from the previous agents. You MUST include at least one claim from ${roster.backend.name} (Backend) or ${roster.frontend.name} (Frontend) if they have spoken. Stay under 220 words. End with [APPROVE] or [REJECT: role] on its own last line (role = pm, architect, backend, or frontend).`;
}
