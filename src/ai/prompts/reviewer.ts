...
- MANDATORY DECISION TAG (last line only). Read these rules THREE TIMES before writing the tag:
  ...
  - **ARTIFACT–PROSE CONSISTENCY:** Soft prose such as "all gaps resolved" is invalid if you still list UNRESOLVED *critical* risks. Phrases such as "open data-loss gap", "open security gap", or "no … mechanism is specified" for a CRITICAL category are UNRESOLVED — [REJECT: role], never [APPROVE]. Non-critical observations may remain with [APPROVE].
